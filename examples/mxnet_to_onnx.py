...
def main(args):
    ...
    converted_model_path = onnx_mxnet.export_model(
        sym, all_args, [input_shape], np.float32, args.output, opset_version=11
    )
    ...
    for input_name in input_map.keys():
        if input_name.endswith("_gamma"):
            ...
            for w in weight_array:
                b.append(w)
            new_nv = helper.make_tensor(
                input_name, TensorProto.FLOAT, [input_dim_val, 1, 1], b
            )
...
if __name__ == "__main__":
    ...
    # general
    parser.add_argument(
        "params", default="./r100a/model-0000.params", help="mxnet params to load."
    )
    parser.add_argument(
        "output", default="./r100a.onnx", help="path to write onnx model."
    )
    ...
    parser.add_argument(
        "--input-mean", default=0.0, type=float, help="input mean for checking."
    )
    parser.add_argument(
        "--input-std", default=1.0, type=float, help="input std for checking."
    )
