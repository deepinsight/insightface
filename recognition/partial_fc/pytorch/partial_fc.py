...
def main(local_rank):
    ...
    if cfg.pf_preload:
        file_model = os.path.join(cfg.pf_preload,str(cfg.local_rank)+'_pf.pth')
        print("Loading pf Checkpoint '{}'".format(file_model))
        loc = 'cuda:{}'.format(cfg.local_rank)        
        dist_sample_classifer.load_state_dict(torch.load(file_model, map_location=loc))
    ...
    optimizer = SGD([{
        ...
        'params': dist_sample_classifer.parameters(),'lr': cfg.lr
    ...
    for epoch in range(start_epoch, n_epochs):
        ...
        for step, (img, label) in enumerate(train_loader):
            ...
            if cfg.local_rank == 0 and step % 50 == 0:
                ...
                print("Speed %d samples/sec   Loss %.4f   Epoch: %d   Global Step: %d lr: %0.6f   Required: %1.f hours" %
                      (
        ...
        for gpu_index in range(cfg.world_size):
            if not os.path.exists(cfg.output):
                os.makedirs(cfg.output)
            torch.save(dist_sample_classifer.state_dict(), os.path.join(cfg.output, str(dist.get_rank()) + '_pf.pth'))
