...
def extract_parallel(prefix, epoch, dataset, batch_size, size):
    ...
    data_loader = DataLoader(dataset, batch_size, last_batch='keep', num_workers=8, thread_pool=True,
        prefetch=64, batchify_fn=batchify_fn)
    symbol, arg_params, aux_params = mx.module.module.load_checkpoint(prefix, epoch)

    # init model list
    for i in range(num_ctx):
        model = mx.mod.Module(symbol, context=mx.gpu(i), label_names=None)
        model.bind(for_training=False, data_shapes=[('data', (2 * batch_size, 3, 112, 112))])
    ...
    for image in tqdm(data_loader):
        ...
        feat = model_list[num_iter % num_model].get_outputs(merge_multi_context=True)[0]
        feat = mx.nd.L2Normalization(feat)
        feat = mx.nd.reshape(feat, (-1, size * 2))
        feat_mat[
        batch_size * num_iter:
        batch_size * num_iter + feat.shape[0], :] = feat.as_in_context(mx.cpu())
        num_iter += 1
        # if num_iter % 20 == 0:
        #     mx.nd.waitall()
...
img_feats = extract_parallel(args.model_prefix, args.model_epoch, dataset, args.batch_size, size=512)
