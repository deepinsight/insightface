...
class ParallModule(BaseModule):
    ...
    def fit(self,
        ...
        # TODO: 不理解为啥要加这个，加了这个中间一不小心中断连岂不是无法继续训练了？
        # assert arg_params is None and aux_params is None

        # 跟之前的输入格式保持一致
        provide_label = [(train_data.provide_label[0][0], (train_data.provide_label[0][1][0],))]
        self.bind(data_shapes=[train_data.provide_data[0][:2]],
        ...
        for epoch in range(begin_epoch, num_epoch):
            ...
            while not end_of_batch:
                ...
                try:
                    ...
                    # TODO: 一旦程序到这就回不去了,train_data.reset()会提示数据没有到结尾无法 reset, orz
                    print("something error raised, start a new epoch, error:")
                    print(e)
                    end_of_batch = True
                    continue
                data_batch = io.DataBatch([batch[0].data[0]], [batch[0].label[0][..., 0]], batch[0].data[0].shape[0])
                if monitor is not None:
                    monitor.tic()
                ...
                # 去除了手动 prefetch，在初始化 dali 时已设置了 prefetch 深度
                # try:
