...
import flops_counter, train_logger
...
def parse_args():
  ...
  parser.add_argument("--log-dir", type=str, default=default.log_dir, help='root directory to save log.')
...
def train_net(args):
    ...
    logging.info('num_classes %s' % config.num_classes)
    path_imgrec = os.path.join(data_dir, "train.rec")

    print('Called with argument:', args, config)
    logging.info('Called with argument: %s %s' % (args, config))
    ...
    if len(args.pretrained)==0:
      ...
      logging.info('loading %s %s' % (args.pretrained, args.pretrained_epoch))
    ...
    def _batch_callback(param):
      ...
      if mbatch%1000==0:
        ...
        logging.info('lr-batch-epoch: %s %s %s' % (opt.lr, param.nbatch, param.epoch))
...
def main():
    global args
    args = parse_args()
    train_logger.loginfo(args.log_dir)
    train_net(args)
