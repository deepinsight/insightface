...
def get_symbol(args):
  ...
  if config.loss_name=='softmax': #softmax
    ...
    if config.loss_m1!=1.0 or config.loss_m2!=0.0 or config.loss_m3!=0.0:
      if config.loss_m1==1.0 and config.loss_m2==0.0:
        ...
        cos_t = zy/s
        theta = mx.sym.arccos(cos_t)
        if config.loss_m1!=1.0:
          t1 = theta*config.loss_m1
        else:
          t1 = theta*1.0
        m2_tar = t1
        if config.loss_m2>0.0:
          t2 = t1+config.loss_m2
        else:
          t2 = t1+0.0
        m2_handler = t2
        body1 = mx.sym.cos(t2)
  ...
  out_list.insert(-1,mx.symbol.BlockGrad(theta))
  out = mx.symbol.Group(out_list)
  handler_list = [m2_tar, m2_handler, m3_tar, m3_handler]
  return out, handler_list

def train_net(args):

    loss_margin = [config.loss_s, config.loss_m1, config.loss_m2, config.loss_m3]
    ctx = []
    ...
    def _batch_callback(param):
      ...
      theta_avg = np.average(model.get_outputs()[2].asnumpy())
      if (np.pi/1.5 < theta_avg and theta_avg <= np.pi):
        loss_margin[2] = 0.0
        loss_margin[3] = 0.0
      elif (np.pi/3 < theta_avg and theta_avg <= np.pi/1.5):
        mar_ratio = (np.pi/1.5 - theta_avg) / (np.pi/1.5 - np.pi/3)
        loss_margin[2] = mar_ratio * 0.6
        loss_margin[3] = mar_ratio * 0.4
      elif (theta_avg <= np.pi/3):
        loss_margin[2] = 0.6
        loss_margin[3] = 0.4
      loss_han[1] = loss_han[0] + loss_margin[2]
      loss_han[3] = loss_han[2] - loss_margin[3]

      if (mbatch%20 == 0):
        print("Adaptive loss margin: m2=%1.4f, m3=%1.4f" % (loss_margin[2], loss_margin[3]))
    ...
    model.fit(train_dataiter,
        ...
        # is this where you can change number of epochs?
        # num_epoch          = 999999,
        num_epoch          = 50,
