...
def image_encode(args, i, item, q_out):
    ...
    header = mx.recordio.IRHeader(0, item.label, item.id, 0)
    oitem = [item.id, item.image_path, item.label]
    if item.aligned:
      ...
      img = face_preprocess.preprocess(img, bbox = item.bbox, landmark=item.landmark, image_size='112,96')
